...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def go(prev: Int, curr: Int, acc: Int): Int =
      if (acc == n) curr
      else go(curr, prev + curr, acc + 1)
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    val neighbours: Array[(A,A)] = as zip as.slice(1, as.length)
    neighbours.foldRight(true)((aa: (A,A), res: Boolean) => gt(aa._1, aa._2) && res)
  }
